...
if log:
    run = neptune.init_run(
        ...
        api_token="eyJhcGlfYWRkcmVzcyI6Imh0dHBzOi8vYXBwLm5lcHR1bmUuYWkiLCJhcGlfdXJsIjoiaHR0cHM6Ly9hcHAubmVwdHVuZS5haSIsImFwaV9rZXkiOiJiYjJhYTNjYy03NjI1LTQzOGQtYjRkYi1mMGE0N2M4NDFkZmUifQ==",
    )  
#"data": 'databrain'
    params = {"learning_rate": 0.0001, "optimizer": "Adam", "epoches": 400,  "iters": 200, "batch_size": 1, "model_name": 'PC_XMorpher_heart_0',  "n_classes": 8, 'data':'Cardiac'
}
...
class RSeg(object):
    def __init__(self, k=0, n_channels=1, n_classes=8, lr=1e-4, epoches=400, iters=200, batch_size=1, model_name='XMorpher_heart_0'):
        ...
        self.results_dir = '/path/to/XMorpher-registration/out/Unsup/results_unsupervised'
        self.checkpoint_dir = '/path/to/XMorpher-registration/out/Unsup/weights_unsupervised'
        self.model_name = model_name
        self.nifti_dir = '/path/to/XMorpher-registration/out/Unsup/nifti_out'
    ...
    def train_iterator(self, mi, fi, ml=None, fl=None, epoch=None):
        global a,b,c
        ...
        if log:
            run["loss_s"].append(float(loss_s))
            run["loss_ncc"].append(float(loss_ncc))
            run["loss_Reg"].append(float(loss_Reg))
            ...
            if epoch % 30 == 0 and epoch!=0:
                ...
                if rad < 10:  # Log images for 5% of batches
                    ...
                    ct_pred_true = np.concatenate((fix[:, :, 50], moving[:, :, 50], moved[:, :, 50]), axis=1)
                    run["fix/moving/moved"].append(File.as_image(ct_pred_true),description=(f"fix/moving/moved at epoch{epoch}"))
    ...
    def train_epoch(self, epoch, is_aug=True):
        ...
        for i in range(self.iters):
            ...
            self.train_iterator(mov_img, fix_img, mov_lab, fix_lab,epoch)
    ...
    def test(self):
        ...
        for i, (mi, fi, ml, fl, name) in enumerate(self.dataloader_labeled_labeled):
           ...
           # print('mov label shape',ml.shape)
            ...
            c = flow
            lab = nib.Nifti1Image(c, affine=None)
            nib.save(lab,os.path.join(self.nifti_dir, 'flow', name[:-4]+'.nii.gz'))
    ...
    def load(self):
        print('the trainig will coninue from: ', '{0}/{1}_epoch_{2}.pth'.format(self.checkpoint_dir, 'Reger_'+self.model_name, str(self.k)))
