...
from utils.utils import EMA, AverageMeter, to_categorical, dice,rescale_array
import numpy as np
...
if log:
    run = neptune.init_run(
        project="medical/XMorpher",
        api_token="",
    )  

    params = {"learning_rate": 1e-4, "optimizer": "Adam", "epoches": 200, "batch_size": 1, "model_name": 'PC_XMorpher_heart_0',  "n_classes": 8
}
    run["parameters"] = params

class RSeg(object):
    def __init__(self, k=0, n_channels=1, n_classes=8, lr=1e-4, epoches=1, iters=1, batch_size=1, model_name='PC_XMorpher_heart_0'):
        ...
        train_labeled_unlabeled_dir = '/path/to/XMorpher-registration/data/train_unlabeled_unlabeled'
        train_unlabeled_unlabeled_dir = '/path/to/XMorpher-registration/data/train_unlabeled_unlabeled'
        test_labeled_labeled_dir = '/path/to/XMorpher-registration/data/test'

        self.results_dir = '/path/to/XMorpher-registration/out/Semi/results_unsupervised'
        self.checkpoint_dir = '/path/to/XMorpher-registration/out/Semi/weights_unsupervised'
        self.model_name = model_name
        self.nifti_dir = '/path/to/XMorpher-registration/out/Semi/nifti_out'
    ...
    def train_iterator_stage0(self, mi, fi, ml, fl, epoch=None):
        """Stage 0: Pretrain segmentation model only on labeled data"""
        # Only train Seger in this stage
        for p in self.Seger.parameters():
            p.requires_grad = True
        for p in self.Reger.parameters():
        ...
        if ml is not None and fl is not None:
            ...
            self.optS.zero_grad()
            if log:
                run["loss_seg_stage0"].append(float(loss_seg))
        ...
        s_ff = s_ff.detach()
        s_mm = s_mm.detach()
        s_ff = torch.argmax(s_ff, dim=1, keepdim=True)
        s_ff = [torch.where(s_ff==i, torch.full_like(s_ff, 1), torch.full_like(s_ff, 0)) for i in range(self.n_classes)]
        s_ff = torch.cat(s_ff, dim=1)
        s_mm = torch.argmax(s_mm, dim=1, keepdim=True)
        s_mm = [torch.where(s_mm == i, torch.full_like(s_mm, 1), torch.full_like(s_mm, 0)) for i in range(self.n_classes)]
        s_mm = torch.cat(s_mm, dim=1)
        ...
        if log:
            ...
            #and epoch != 0
            if epoch % 20 == 0:
                rad = np.random.randint(low=0, high=100)
                if rad < 10:  # Log images for 10% of batches
                    seg1 = fl
                    seg2 = s_ff
                    seg3 = ml
                    seg4 = s_mm

                    fix_lab = rescale_array(seg1.squeeze().detach().cpu().numpy(), 0, 1)
                    fPred_lab = rescale_array(seg2.squeeze().detach().cpu().numpy(), 0, 1)
                    moving_lab = rescale_array(seg3.squeeze().detach().cpu().numpy(), 0, 1)
                    mPred_lab = rescale_array(seg4.squeeze().detach().cpu().numpy(), 0, 1)

                    slice_dict = {
                        1: 95, 2: 95, 3: 122, 4: 122, 5: 40,
                        6: 74, 7: 95, 8: 122, 9: 20, 10: 20,
                        11: 116, 12: 116
                    }

                    for i, z in slice_dict.items():
                        ct_pred_true = np.concatenate((
                            fix_lab[i, :, :, z],
                            fPred_lab[i, :, :, z],
                            moving_lab[i, :, :, z],
                            mPred_lab[i, :, :, z]
                        ))
                        run[f"Stage0:F_lab predF_Lab/5mLab/mpredL/Label{i}"].append(
                            File.as_image(ct_pred_true.T),
                            description=f"Label {i} at slice {z}, epoch {epoch}"
                        )


    def train_iterator_stage1(self, mi, fi, ml=None, fl=None, epoch=None):
        # train Seger
        ...
        if ml is not None and fl is not None:
            ...
            self.optS.zero_grad()
            if log:
                run["loss_seg_stage1"].append(float(loss_seg))
        ...
        self.optR.zero_grad()
        if log:
            run["loss_Reg_stage1"].append(float(loss_Reg))
            run["loss_s_stage1"].append(float(loss_s))
            run["loss_ncc_stage1"].append(float(loss_ncc))
            ...
            if epoch % 15 == 0 and epoch!=0:
                ...
                if rad < 10:  # Log images for 10% of batches
                    ...
                    fix = rescale_array((((fi.squeeze().detach()).cpu()).numpy()),0,1) 
                    moving = rescale_array((((mi.squeeze().detach()).cpu()).numpy()),0,1) 
                    moved = rescale_array((((a.squeeze().detach()).cpu()).numpy()),0,1)

                    ct_pred_true = np.concatenate((fix[:, :, 60], moving[:, :, 60], moved[:, :, 60]), axis=1)
                    
                    run["Stage1:fix/moving/moved"].append(File.as_image(ct_pred_true),description=(f"Stage1:fix/moving/moved at epoch{epoch}"))


    def train_iterator_stage2(self, mi, fi, ml=None, fl=None, epoch=None):
        for p in self.Seger.parameters():  # reset requires_grad
            p.requires_grad = False  # they are set to False below in netG update
        for p in self.Reger.parameters():  # reset requires_grad             -
            p.requires_grad = True  # they are set to False below in netG update
        ...
        self.optR.zero_grad()
        if log:
            run["loss_Reg_stage2(loss_s + loss_ncc)"].append(float(loss_Reg))
            run["loss_Reg_stage2(loss_s)"].append(float(loss_s))
            run["loss_Reg_stage2(loss_ncc)"].append(float(loss_ncc))
        ...
        if ml is not None and fl is None:
            ...
            if log:
                run["loss_Seg_stage2(loss_anchor1 + 0.5*loss_w)"].append(float(loss_seg))
                run["loss_stage2(loss_anchor1)"].append(float(loss_anchor1))
                run["loss_stage2(loss_w)"].append(float(loss_w))
                
                if epoch % 15 == 0:
                    rad = np.random.randint(low=0, high=100)
                    if rad < 10:  # Log images for 5% of batches
                        a = moved
                        fix = rescale_array((((fi.squeeze().detach()).cpu()).numpy()),0,1) 
                        moving = rescale_array((((mi.squeeze().detach()).cpu()).numpy()),0,1) 
                        moved = rescale_array((((a.squeeze().detach()).cpu()).numpy()),0,1)
                        ...
                        ct_pred_true = np.concatenate((fix[:, :, 60], moving[:, :, 60], moved[:, :, 60]), axis=1)
                        run["Stage2:fix/moving/moved"].append(File.as_image(ct_pred_true),description=(f"Stage2:fix/moving/moved at epoch{epoch}"))
    ...
    def train_epoch_stage1(self, epoch, is_aug=True):
        ...
        for i in range(self.iters):
            ...
            self.train_iterator_stage1(mov_img, fix_img, mov_lab, fix_lab, epoch=epoch)
    ...
    def train_epoch_stage2(self, epoch, is_aug=True):
        ...
        for i in range(self.iters):
            ...
            self.train_iterator_stage2(mov_img, fix_img, mov_lab, fix_lab, epoch=epoch)
            res = '\t'.join(['Epoch: [%d/%d]' % (epoch + 1, self.epoches),
                             'Iter: [%d/%d]' % (i + 1, self.iters),
                             self.L_smooth_log.__str__(),
                             self.L_ncc_log.__str__(),
    ...
    def test_iterator(self, mi, fi, ml=None, fl=None):
        with torch.no_grad():
            ...
            r_mi = s_m * mi
            r_mi = r_mi[:, 1:, :, :, :]
            # print('the moving label shape in test:', ml.shape)
            w_m_to_f, w_f_to_m, w_label_m_to_f, w_label_f_to_m, flow = self.Reger(r_mi, r_fi, ml, fl)
    ...
    def test(self):
        ...
        for i, (mi, fi, ml, fl, name) in enumerate(self.dataloader_labeled_labeled):
            ...
            if not os.path.exists(join(self.nifti_dir, 'mi')):
                os.makedirs(join(self.nifti_dir, 'mi'))
            if not os.path.exists(join(self.nifti_dir, 'fi')):
                os.makedirs(join(self.nifti_dir, 'fi'))
            if not os.path.exists(join(self.nifti_dir, 'ml')):
                os.makedirs(join(self.nifti_dir, 'ml'))
            if not os.path.exists(join(self.nifti_dir, 'fl')):
                os.makedirs(join(self.nifti_dir, 'fl'))
            if not os.path.exists(join(self.nifti_dir, 'w_m_to_f')):
                os.makedirs(join(self.nifti_dir, 'w_m_to_f'))
            if not os.path.exists(join(self.nifti_dir, 'flow')):
                os.makedirs(join(self.nifti_dir, 'flow'))
            if not os.path.exists(join(self.nifti_dir, 'w_m_to_f')):
                os.makedirs(join(self.nifti_dir, 'w_m_to_f'))
            if not os.path.exists(join(self.nifti_dir, 'w_label_m_to_f')):
                os.makedirs(join(self.nifti_dir, 'w_label_m_to_f'))
            if not os.path.exists(join(self.nifti_dir, 's_m')):
                os.makedirs(join(self.nifti_dir, 's_m'))
            if not os.path.exists(join(self.nifti_dir, 's_f')):
                os.makedirs(join(self.nifti_dir, 's_f'))

            b = mi
            ct = nib.Nifti1Image(b, affine=None)
            nib.save(ct,os.path.join(self.nifti_dir, 'mi', name[:-4]+'.nii.gz'))

            b = fi
            ct = nib.Nifti1Image(b, affine=None)
            nib.save(ct,os.path.join(self.nifti_dir, 'fi', name[:-4]+'.nii.gz'))

            b = ml
            ct = nib.Nifti1Image(b, affine=None)
            nib.save(ct,os.path.join(self.nifti_dir, 'ml', name[:-4]+'.nii.gz'))


            b = w_m_to_f
            ct = nib.Nifti1Image(b, affine=None)
            nib.save(ct,os.path.join(self.nifti_dir, 'w_m_to_f', name[:-4]+'.nii.gz'))

            c = w_label_m_to_f
            lab = nib.Nifti1Image(c, affine=None)
            nib.save(lab,os.path.join(self.nifti_dir, 'w_label_m_to_f', name[:-4]+'.nii.gz'))

            b = fl
            ct = nib.Nifti1Image(b, affine=None)
            nib.save(ct,os.path.join(self.nifti_dir, 'fl', name[:-4]+'.nii.gz'))
    ...
    def evaluate(self):
        ...
        image_filenames = listdir(join(self.nifti_dir, 's_f'))

        for i in range(len(image_filenames)):
            name = image_filenames[i]
            w_label_m_to_f = np.fromfile(join(self.nifti_dir, 'w_label_m_to_f', name), dtype=np.float32)
            w_label_m_to_f = to_categorical(w_label_m_to_f, self.n_classes)
            fl = np.fromfile(join(self.nifti_dir, 'fl', name), dtype=np.float32)
            fl = to_categorical(fl, self.n_classes)
            ml = np.fromfile(join(self.nifti_dir, 'ml', name), dtype=np.float32)
            ml = to_categorical(ml, self.n_classes)
            s_m = np.fromfile(join(self.nifti_dir, 's_m', name), dtype=np.float32)
