...
def imgnorm(N_I, index1=0.01, index2=0.01):
...
class DatasetFromFolder3D(data.Dataset):
    ...
    def __getitem__(self, index):
        # 读取image和label
        print('This is the data name:',self.image_filenames[index])
        data = loadmat(join(self.file_dir, self.image_filenames[index]))

        fix_img = data['fix_img']

        fix_img = np.where(fix_img < -20., -20., fix_img) 
        fix_img = np.where(fix_img > 5., 5., fix_img)
        fix_img = (fix_img +5. ) / 20.
        ...
        mov_img = np.where(mov_img < -20., -20., mov_img)
        mov_img = np.where(mov_img >  5., 5., mov_img)
        mov_img = (mov_img +5. ) / 20.
